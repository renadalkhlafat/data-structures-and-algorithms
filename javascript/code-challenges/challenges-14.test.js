...
const screenForNames = (arr) =>arr.map((element) => element.match(/^(Mr|Mrs|Ms|Dr|)\..[A-Za-z]+$/g)).filter((element) => element).flat();
...
const toTitleCase = (arr) => {
  // Solution code here...
  let newArray=arr.map((element) => element[0].toUpperCase() + element.slice(1));
  return newArray;
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let result=arr.reduce((acc, curr) => acc + (curr.mass > 77 ? ' - ' + curr.name : ''), '').slice(3);
