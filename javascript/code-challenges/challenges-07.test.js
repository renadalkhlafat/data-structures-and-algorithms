...
const removeThree = (idx, arr) => {
  let deletedItems = arr.splice(idx, 3);
  return arr.filter(item => !deletedItems.includes(item));
};
...
const howMuchPencil = (str) => {
  ...
  for (let index = 0; index < str.length; index++) {
    let str2 = result[index];
    result.push(str2.slice(1));
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < ingredients.length; i++) {
    ...
    for (let j = 0; j < mesure.length; j++) {
      value = value.replace(mesure[j], '');
    }
    result.push(value);
  }
  return result;
...
const extractVowels = (str) => {
  ...
  let al = '';
  for (let index = 0; index < str.length; index++) {
    !vowels.includes(str[index]) ? result += str[index] : al += str[index];
